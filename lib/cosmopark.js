...
const coordinator_1 = require("./relayers/coordinator");
class Cosmopark {
    context;
    filename;
    logLevel = 'info';
        ...
        for (const [index, relayer] of Object.entries(config.relayers || [])) {
            switch (relayer.type) {
                ...
                case 'neutron':
                    ...
                    break;
                case 'coordinator':
                    instance.relayers.push(new coordinator_1.CosmoparkCoordinatorRelayer(`relayer_${relayer.type}${index}`, relayer.type, instance.filename));
                    break;
                default:
    ...
    async stopRelayer(type, index) {
        await docker_compose_1.default.stopOne(`relayer_${type}${index}`);
    }
    ...
    executeInNetwork = (network, command) => this.networks[network].execInSomewhere(command);
    stop = async () => {
    ...
    generateDockerCompose = () => {
        ...
        for (const [index, relayer] of Object.entries(this.config.relayers || [])) {
            ...
            if (relayer.type === 'hermes') {
                services[name] = {
                    ...
                    volumes: [`${name}:/root`],
                    entrypoint: ['/bin/bash'],
                    depends_on: relayer.networks.map((network) => `${network}${this.config.networks[network].type === 'ics' ? '_ics' : '_val1'}`),
                };
                volumes[name] = null;
                ...
                const targetNetwork = relayer.networks.find((network) => this.config.networks[network].type !== 'ics');
                for (const [network1, network2] of prevConnections) {
                    if ((network1 === icsNetwork && network2 === targetNetwork) ||
                ...
                let environment = {
                    ...
                    RELAYER_TARGET_CHAIN_ACCOUNT_PREFIX: this.config.networks[targetNetwork].prefix,
        ...
        fs_1.default.writeFileSync(this.filename, yaml_1.default.stringify(dockerCompose, { indent: 2, lineWidth: -1 }));
    };
